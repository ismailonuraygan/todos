...
const Todo = ({ todo }) => {
  ...
  return (
    <div className="w-auto min-w-[25%] mx-auto space-y-6 max-w-md flex flex-col items-stretch">
      {updateMode ? (
        <form
          className="border-b border-gray-600 flex flex-col"
          onSubmit={(e) => {
            e.preventDefault();
            updateTodo(todo.id);
          }}
        >
              <input
                ...
                placeholder="Title"
                value={form.title}
                className="border-2 rounded border-gray-500 p-1 mb-2"
                onChange={(e) => setForm({ ...form, title: e.target.value })}
              />
              <textarea
                placeholder="Content"
                value={form.content}
                className="border-2 rounded border-gray-500 p-1 flex-1 mb-2"
                onChange={(e) => setForm({ ...form, content: e.target.value })}
              />
            <button type="submit" className="border-2 bg-green-500 px-3 mb-1">
              <CheckIcon />
            </button>
        </form>
      ) : (
